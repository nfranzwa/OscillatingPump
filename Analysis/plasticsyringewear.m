...
mu = 0.08;               % Friction coefficient for PP-PTFE (slightly higher than glass-PTFE)
hardness_ptfe = 58.2;    % Shore D hardness

% Polypropylene Syringe
E_plastic = 1.5e9;        % Young's modulus (Pa) for polypropylene
v_plastic = 0.42;         % Poisson's ratio for polypropylene
rho_plastic = 946;        % Density (kg/m^3)
cp_plastic = 1920;        % Specific heat (J/kg·K)
k_plastic = 0.12;         % Thermal conductivity (W/m·K)
...
t_breakin = 1.5*24*3600;   % 2 day break-in period (shorter for PP-PTFE)
K_initial = 85e-15;      % Initial wear rate for PP-PTFE (higher than glass-PTFE)
K_steady = 12e-15;       % Steady-state wear rate for PP-PTFE (higher than glass-PTFE)

% Calculate total cycles
total_cycles = freq * duration;

% Calculate effective elastic modulus (plane strain)
E_eff = 1 / ((1-v_ptfe^2)/E_ptfe + (1-v_plastic^2)/E_plastic);
...
title('Plastic Seal Contact Pressure vs Time', 'Interpreter', 'latex');
