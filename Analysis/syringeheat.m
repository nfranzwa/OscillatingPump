function [T_history, Q_total] = analyzeSyringeHeat(params)
    ...
    T_wall = zeros(1, n_steps);   % Glass wall temperature
    ...
    mass_glass = rho_glass * params.glass_volume;
    ...
    for i = 2:n_steps
        ...
        dT_wall = (Q_air_to_wall + Q_seal_to_wall - Q_wall_to_ambient) * dt / (mass_glass * cp_glass);
...
params.friction_coef = 0.05;         % PTFE-glass friction
params.duration = 250;               % 250 seconds simulation

% Measured geometry parameters
params.glass_volume = 1.34456e-6;    % m³ glass volume (calculated from dimensions)
params.seal_volume = 1.38918e-6;     % m³ PTFE seal volume (provided)
...
title('Air Temperature Inside Glass Syringe', 'Interpreter', 'latex');
grid on;
